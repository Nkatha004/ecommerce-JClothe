...
<div class="w3-sidebar w3-light-grey w3-bar-block" style="width:21%; padding-left:10px">
		...
		<?php 
			...
			$query = $model_cat->query($sql_cat);
			if($res = $query->getResult()){
				foreach($res as $row){ 

					?><label><input type="checkbox" name ="category" value = <?php echo $row->category_id;?>><?php echo $row->category_name;?></label><br><?php
				}
			}
		?>
...
<main>
    ...
    <nav class="navbar navbar-dark bg-dark">
        <div class="container-fluid">ANALYTICS - Category purchases
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#remove_filter").click(function(){
			window.location.href = "/clothes/purchases_per_category";
		});
            	...
            	success: function(data){
            		...
            		if(data != ''){
               			...
               			for(i = 0; i < data.length; i++){
            				...
            				msg += '<tr><td>'+data[i]["order_id"]+'</td><td>'+data[i]["product_id"]+'</td><td>'+data[i]['name']+'</td><td>'+data[i]["catname"]+'</td><td>'+data[i]['quantity']+'</td><td>'+data[i]['total']+'</td></tr>';
